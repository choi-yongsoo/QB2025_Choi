...
rm(list = ls())
getwd()
setwd("/cloud/project/QB2025_Choi/Week1-RStudio")
```
...
> Answer 1: it generates random number which follow normal distribution with parameters (mean and sd) that I put. 
...
```{r}
m <- read.table("data/matrix.txt", sep ="\t", header = TRUE)
n <- t(m)
dim(n)
```
...
> Answer 2:how many rows and cols are in the matrix.
...
> Answer 3: It compares each measured value to the others to show how correlated they are. By doing this, we can easily recognize which values are highly correlated at once. 
...
> Answer 4: I think these data is quite robust whether I used parametric or nonparametric methods, but some values show different significance in terms of p-value .005 (e.g. TP vs. SRP). As I know, if the values in data cannot satisfy equal variance test or normalization test, we should use non parametric test, which means we need a good-quality data for performing parametric tests. Next, for FDR, our data seems fine with it (I mean comparing those p-values with .005, there is no p-values which became higher than .005 after BH correction in our data here), but the p-values are definitely higher after BH correction, so I think we should do this FDR correction to prevent concluding false positive. When testing multiple hypotheses simultaneously, the risk of false-positive conclusions increases with the number of comparisons. Therefore, it is necessary to adjust p-values to reduce the false discovery rate.
...
plot(meso$TN, meso$ZP, ylim = c(0, 10), xlim = c(500, 5000), 
     ...
     ylab = "Zooplankton Biomass (mg)",  las = 1)
...
> Answer 5: The zooplankton biomass has significant poitive effects on the concentration of nitrogen with the p-value of 1.911e-05. And this regresion model explains about 55% of variability of nitrogen concentration by zooplankton biomass, which I believe not very high. 
...
```{r}
sbs <- read.table("data/zoops.txt", sep = "\t", header = TRUE)
sbs2 <- sbs[ ,c(3:11)]
sbs2
boxplot(sbs2, 
        xlab = "Species",
...
biomass.total <- colSums(sbs2)
biomass.total
sbs3 <- rbind(sbs2, biomass.total)
sbs4 <- t(sbs3)
barplot(sbs4[,25], 
        xlab = "Species",
        ylab = "Total biomass")

stack <- aggregate(cbind(CAL, DIAP, CYCL, BOSM, SIMO, CERI, NAUP) ~ NUTS, data = sbs, FUN = sum)
stacked_data <- as.matrix(stack[, -1])
barplot(
  t(stacked_data),                     ,                     
  col = rainbow(ncol(stacked_data)),  
  xlab = "Nutrient supply",
  ylab = "Zooplankton biomass",
  names.arg = c("low", "medium", "high"))
legend("topright", legend = colnames(stacked_data), fill = rainbow(ncol(stacked_data)), cex = 0.5, ncol = 7)
```
